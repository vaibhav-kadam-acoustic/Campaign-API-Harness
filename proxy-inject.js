 ...
 * Proxy is used implicitly — no UI field needed.
 */
(function () {
  'use strict';
  ...
  // ─── 1. PATCH fetchToken TO ROUTE THROUGH PROXY ───────────────────────────
  ...
  // ─── 2. INJECT RUN BUTTON VIA MutationObserver ────────────────────────────
  ...
  function injectRunUI(outputPane) {
    ...
    panel.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">' +
      ...
      'style="margin-left:auto;padding:3px 8px;background:transparent;border:1px solid var(--line,#d8d2c8);border-radius:6px;cursor:pointer;font-size:10px;color:var(--muted,#5c5040)">✕</button>' +
      '</div>' +
    ...
    if (curl && curl.nextSibling) {
      outputPane.insertBefore(panel, curl.nextSibling);
    } else {
      outputPane.appendChild(panel);
    }
  }
  ...
  async function runViaProxy() {
    ...
    if (op.apiType === 'xml') {
      ...
      endpoint    = pod + (builtR.url || '');
      method      = builtR.method || op.method || 'GET';
      contentType = 'application/json';
      body        = builtR.body || null;
    }
